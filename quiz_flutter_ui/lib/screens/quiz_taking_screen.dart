...
class _TakingExamScreenState extends State<TakingExamScreen> {
  ...
  Future<void> _initQuestions() async {
    ...
    final saveFile = File(p.join(settings.workspacePath, 'current_session.json'));
    ...
    if (savedState != null && mounted) {
      final resume = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: const Text('Hệ thống tìm thấy bài thi đang làm dở của đề này. Bạn có muốn tiếp tục không?'),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Làm mới')),
      ...
      if (resume == true) {
        setState(() {
          _questions = List<dynamic>.from(savedState?['questions'] ?? rawQuestions);
          (savedState?['selected_answers'] as Map<String, dynamic>).forEach((k, v) {
            _selectedAnswers[int.parse(k)] = v;
          });
          (savedState?['flagged_questions'] as List<dynamic>).forEach((id) {
  ...
  Future<void> _autoSave() async {
    try {
      ...
      final state = {
        ...
        'selected_answers': _selectedAnswers.map((k, v) => MapEntry(k.toString(), v)),
  ...
  void _submitExam() async {
    ...
    try {
      ...
      await db.saveAttempt(
        ...
        answersJson: jsonEncode(_selectedAnswers.map((k, v) => MapEntry(k.toString(), v))),
      );
    } catch (e) {}
  ...
  void _confirmSubmit() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Bạn đã trả lời ${_selectedAnswers.length}/${_questions.length} câu. Bạn có chắc chắn muốn nộp bài?'),
        actions: [
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      IconButton.filledTonal(
                        ...
                        icon: Icon(
                          _flaggedQuestions.contains(currentQuestion['id']) ? Icons.flag : Icons.flag_outlined,
                          color: _flaggedQuestions.contains(currentQuestion['id']) ? Colors.orange : null,
                        ),
                  ...
                  ...((currentQuestion['options'] as Map<String, dynamic>? ?? {}).entries.map((entry) {
                    final label = entry.key;
                    final text = entry.value;
                    final isSelected = _selectedAnswers[currentQuestion['id']] == label;
                          ...
                          child: Row(
                            children: [
                              Radio<String>(
                                ...
                                onChanged: (val) {
                                  setState(() {
                                    _selectedAnswers[currentQuestion['id']] = val!;
                                  });
                                  _autoSave();
                                },
                              ),
